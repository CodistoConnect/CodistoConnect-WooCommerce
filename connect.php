					...
					if ( ! $order_id ) {
						...
						foreach ( $ordercontent->orderlines->orderline as $orderline ) {
							if ( $orderline->productcode[0] != 'FREIGHT' ) {
								...
								foreach ( $orderline->attributes->attribute as $attribute ) {
									$nodeAttributes = $attribute->attributes();
									$attributename = (string) $nodeAttributes['name'];
									$attributevalue = (string) $nodeAttributes['value'];
									wc_add_order_item_meta( $item_id, $attributename, $attributevalue );
								}
								$tax += $line_total_tax;
